...
FPTree::FPTree(const std::vector<Transaction> &transactions, uint64_t minimum_support_threshold) : root(std::make_shared<FPNode>(Item{}, nullptr)), header_table(),
...
std::set<Pattern> fptree_growth(const FPTree &fptree)
{
    ...
    else
    {
        ...
        for (const auto &pair : fptree.header_table)
        {
            ...
            for (const auto &pattern : curr_item_patterns)
            {
                if (pattern.second >= fptree.minimum_support_threshold)
                {
